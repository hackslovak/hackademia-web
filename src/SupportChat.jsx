...
export default function SupportChat() {
  ...
  return (
    <div style={{ position: 'fixed', bottom: '30px', right: '30px', zIndex: 9999, display: 'flex', flexDirection: 'column', gap: '12px', alignItems: 'flex-end', fontFamily: 'sans-serif' }}>
      <style>{`
        .support-trigger-btn { width: 55px; height: 55px; border-radius: 50%; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 15px rgba(0,0,0,0.15); transition: all 0.3s ease; background-color: #062440; color: #ffffff; }
        .support-trigger-btn:hover { transform: translateY(-4px) scale(1.05); box-shadow: 0 10px 25px rgba(255,123,84,0.35); background-color: #FF7B54; }
        .auth-btn { width: 100%; display: flex; align-items: center; justify-content: center; gap: 10px; padding: 12px; border-radius: 12px; border: none; cursor: pointer; font-weight: bold; font-size: 14px; transition: 0.2s; margin-bottom: 10px; }
          ...
          <div style={{ padding: '20px', flex: 1, minHeight: '300px', display: 'flex', flexDirection: 'column', overflowY: 'hidden' }}>
            {!authUser && view === 'main' && (
              <div style={{ animation: 'fadeIn 0.3s ease' }}>
                <div style={{ background: '#F4F7F6', padding: '15px', borderRadius: '15px 15px 15px 4px', fontSize: '14px', color: '#2D3748', lineHeight: '1.5', marginBottom: '20px' }}>{t('authRequired')}</div>
                <button onClick={handleTelegramLogin} className="auth-btn tg"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 2L11 13M22 2l-7 20-4-9-9-4 20-7z" /></svg>{t('loginTelegram')}</button>
                <button onClick={handleGoogleLogin} className="auth-btn google"><svg viewBox="0 0 24 24" width="18" height="18" xmlns="http://www.w3.org/2000/svg"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>{t('loginGoogle')}</button>
              </div>
            )}
                ...
                <div style={{ flex: 1, overflowY: 'auto', marginBottom: '15px', display: 'flex', flexDirection: 'column', gap: '8px', paddingRight: '5px' }}>
                  ...
                  {chatHistory.map((msg, i) => (
                    <div key={i} style={{ 
                      ...
                      background: msg.sender === 'user' ? '#FF7B54' : '#F4F7F6', 
                      color: msg.sender === 'user' ? '#fff' : '#2D3748', 
                      padding: '10px 14px', 
                      borderRadius: msg.sender === 'user' ? '15px 15px 4px 15px' : '15px 15px 15px 4px', 
                      maxWidth: '85%', 
                      fontSize: '13px' 
                    }}>
                      {msg.text}
                  ...
                  {isTyping && (<div style={{ alignSelf: 'flex-start', background: '#F4F7F6', padding: '10px 14px', borderRadius: '15px 15px 15px 4px', fontSize: '12px', color: '#718096', fontStyle: 'italic', animation: 'fadeIn 0.3s ease' }}>{t('typing')}</div>)}
                  {systemMsg && (<div style={{ alignSelf: 'center', background: '#FFFBEB', border: '1px solid #FDE68A', padding: '8px 12px', borderRadius: '8px', fontSize: '11px', color: '#92400E', textAlign: 'center', marginTop: '10px', animation: 'fadeIn 0.5s ease' }}>{systemMsg}</div>)}
                  <div ref={chatEndRef} />
                </div>
            ...
            {view === 'telegram' && (
              <div style={{ textAlign: 'center', animation: 'fadeIn 0.5s ease', padding: '15px 0' }}>
                ...
                <p style={{ color: '#718096', fontSize: '13px', lineHeight: '1.5', background: '#F4F7F6', padding: '12px', borderRadius: '10px' }}>{t('guestReply')}</p>
                <button onClick={() => { closeChat(); setTimeout(() => setView('main'), 500); }} style={{ background: '#1A3636', color: '#fff', border: 'none', borderRadius: '12px', padding: '10px 20px', marginTop: '15px', cursor: 'pointer', fontWeight: 'bold', width: '100%' }}>{t('close')}</button>
              </div>
            )}
          </div>
        </div>
